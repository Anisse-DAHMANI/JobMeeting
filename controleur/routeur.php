<?php
require_once 'controleurAfficherCsv.php';
require_once 'controleurAuthentification.php';
///PROJET
require_once 'controleurAfficherCsvResponsable.php';
///
require_once 'controleurAfficherCsvResponsable.php';
...
require_once 'controleurGestionResp.php';
...
class Routeur {
  ...
  private $ctrlAfficherCsv;
  private $ctrlAfficherCsvResponsable;
  ...
  */
  public function __construct() {
	...
	$this->ctrlAfficherCsv = new ControleurAfficherCsv();
	...
	$this->ctrlAfficherCsvResponsable = new ControleurAfficherCsvResponsable();
	 $this->ctrlGestResp = new controleurGestionResp();
    $this->dao = new Dao_2016();
  }

  // Traite une requête entrante
  public function routerRequete() {
   if(isset($_POST['supprimerResp']) && isset($_POST["mailsResp"]))
   {
     $this->dao->supprimerResp($_POST["mailsResp"]);
     $this->ctrlMenu->afficherMenu(2);
     return;
   }
   if(isset($_POST['creer_responsable'])){
    
     $password = $_POST['mdpresp'] ;
     ...
     /*if(!$uppercase || !$lowercase || !$number || !$specialChars || strlen($password) < 8) {
       ...
       $this->ctrlGestResp->genererVueAjoutResp_ctrl();
       $forms = $_POST['formations'];
       ...
       return; 
         ...
         $mdp_crypt = crypt($password, "$6$".$salt) ;
         $nom = $_POST['nomResp'];
         $prenom = $_POST['prenomResp'];
         $mail = $_POST['email'];
         $num = $_POST['numTelResp'];
         $forms = $_POST['formations'];
         $this->dao->ajoutResponsable($nom, $prenom,$mail, $num, $mdp_crypt);
         $this->dao->ajoutLienFormationResp($mail, $forms);
         $this->ctrlGestResp->ajouterResp();
         return;
      /* }*/
   }
   if(isset($_POST['creer_etudiant'])){
     ...
     /*if(!$uppercase || !$lowercase || !$number || !$specialChars || strlen($password) < 8) {
       ...
       $this->ctrlGestResp->genererVueAjoutEtu_ctrl();
       $forms = $_POST['formations'];
       $entre = $_POST['entreprises'];
       echo('Password should be at least 8 characters in length and should include at least one upper case letter, one number, and one special character.');
       return; 
     }else{*/
         $salt = chr(rand(48,122));
         ...
         for ($i = 0; $i < count($entre); $i++) {
           //echo str_replace(' ', '_', $entre[$i]); 
             $str.= $this->dao->getIdEntreprise(str_replace('_', ' ', $entre[$i]));
             $str.=",";
       /*}*/
       
         $this->dao->ajoutEtudiantBis($nom, $prenom,$mail, $num, $mdp_crypt, $forms, $str);
         $this->ctrlGestResp->ajouterEtu();
   ...
   if(isset($_POST['creer_entreprise'])){
     ...
     /*if(!$uppercase || !$lowercase || !$number || !$specialChars || strlen($password) < 8) {
       ...
       return; 
         ...
         $mdp_crypt = crypt($password, "$6$".$salt) ;
         $nomEnt = $_POST['nomResp'];
         $nomContact = $_POST['nomContact'];
         $prenomContact = $_POST['prenomContact'];
         $mail = $_POST['email'];
         $num = $_POST['numTelResp'];
         $adresse = $_POST['adresseEnt'];
         $ville = $_POST['villeEnt'];
         $cpEnt = $_POST['cpEnt'];
         $forms = $_POST['formations'];
         $nbRecru = $_POST['nbRecruteurs'];
         $nbPlaces = $_POST['nbPlaces'];
         $nbStands = $_POST['nbStands'];
         $nbRepas = $_POST['nbRepas'];
       
         $this->dao->ajoutEntBis($nomEnt, $nomContact, $prenomContact,$mail, $num, $adresse, $ville, $cpEnt, $nbRecru,$nbPlaces, $nbStands, $nbRepas, $mdp_crypt, $forms);           $this->ctrlGestResp->ajouterEnt();
         $this->ctrlGestResp->ajouterEnt();
         return;
       /*}*/
   }
    ...
    if(isset($_POST['changementListeEtu'])) {
	  ...
	  if ($_POST['choix5'] != "Faire un choix..." && $this->dao->getNbPlacesRestantes($_POST['choix4'],$formationEtudiant) != -1 ) {
        $string5 = ','.$_POST['choix5'];
      }

      $newList = $string1.$string2.$string3.$string4.$string5;
 ...
 ///PROJET
   if(isset($_GET["export"]) && $_SESSION['type_connexion'] == "responsable"){
     $this->ctrlAfficherCsvResponsable->afficherCsv($_GET["export"]);
     return;
   }
   ///
   ...
   if(isset($_POST["ajouter"])){
     $this->ctrlGestResp->genererVueAjoutEnt_ctrl();
     return;
   }
   else
   {
       ...
       if(isset($_POST['mail'])){
         ...
         $this->ctrlMail->afficherFormulaireMail(-1); //21/01
         return;
       ...
       if(isset($_GET['suppressionFormation'])){
         ...
         return;
          ...
          $this->ctrlAfficherCsv->afficherCsv($_GET["export"]);
          return;
        }
		//PROJET
       if ($_SESSION['type_connexion'] == "responsable") {
       if(isset($_GET['affichageFormation'])){
         $this->ctrlMenu->afficherUneFormation($_GET['affichageFormation']);
         return;
       }
    ...
    if (isset($_GET['profil']) && isset($_GET['type']) && isset($_SESSION['type_connexion'])) {
	  ...
	  //PROJET
     if ($_GET['type'] == "responsable") {
       $_GET['profil'] = $this->dao->my_simple_crypt( $_GET['profil'], 'd' );
       $this->ctrlProfil->afficherProfil("responsable",$this->dao->getResp($_GET['profil']));
       return;
     }
     ///
    }
  ...
  if (isset($_POST['changementConfig'])) {
    ...
    if(isset($_POST['modification_choixvisio'])){
        if($_POST['visio'] != ""){
            $this->dao->editVisioEntreprise(($_SESSION['idUser']), $_POST['visio']);
        }
    }

/*    if(isset($_POST['visioconf'])){
        header('Location : http://localhost:3000/views/room');
    }*/
      ...
      if ($_SESSION['type_connexion'] == "admin") {
        ...
        foreach ($tabFormation as $key => $value) {
          ...
          if (isset($_POST['debEntreModif_'.$idFormation])) {
            ...
            $numjurie = $_POST["jurieModif_$idFormation"];
            $tabjurie=$this->dao->getjurie();
            $this->dao->editCreneauDebutEtFinFormationEntreprise($idEntre, $numDebut , $numFin , $idFormation);
            if (isset($tabjurie[intval($idFormation)])){
              $this->dao->updatejurie($idFormation,$numjurie);
            } else {
              $this->dao->setjurie($idFormation,$numjurie);
            }
          }
        }
      }
    ...
    if (isset($_POST['ajoutCreneauxFormationEntreprise'])) {
      if ($_SESSION['type_connexion'] == "admin") {

        $idEntre = $_SESSION['idUser'];
        $formation = $_POST["selectformation"];
        $numDebut = $_POST["creneauDebut"];
        $numFin = $_POST["creneauFin"];
        $numjurie = $_POST["jurieAdd"];

        $this->ctrlProfil->ajoutcreneauxFormationEntrepise($idEntre, $formation, $numDebut, $numFin, $numjurie);
