...
<?php
	if ($type=="etudiant") {
	    ...
	    border: 1px solid navy;
			...
			?>
				...
				$id=$etudiantCourant->getID();
				$listeEntreprises = $dao->getEntreprisesParFormationBis($etudiantCourant->getFormationEtu());
    ...
    $id = $profil->getID();
		...
		if ($_SESSION['type_connexion']=="etudiant") {
				$forma=$dao->getFormationEtudiant($_SESSION['idUser']);
			
			$chemin = "offre/$nomEnt"."_offre_"."$forma".".pdf";
			if (file_exists($chemin)) {	?>
				<a name="lien_offre" href="offre/<?php echo $nomEnt ?>_offre_<?php echo $forma?>.pdf"> <i> Télécharger l'offre d'emploi <?php echo $forma;  ?> </i> </a> <br/>
				<?php
			}}
		else 
